...
struct Phoenix {
  ...
  class Binding {
    var event: String
    var callback: Phoenix.Message -> Void?
    
    init(event: String, callback: Phoenix.Message -> Void?) {
      (self.event, self.callback) = (event, callback)
      create()
    }
    
    func create() -> (String, Phoenix.Message -> Void?) {
  ...
  class Channel {
    ...
    func on(event: String, callback: (Phoenix.Message -> Void)) {
